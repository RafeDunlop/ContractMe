...
//        when(taskService.getTaskById(1L)).thenReturn(renovationTask);
//
//        RenovationTaskValidation renovationTaskValidation = new RenovationTaskValidation();
//        ReflectionTestUtils.setField(taskService, "renovationTaskValidation", renovationTaskValidation);
//
//        Mockito.doCallRealMethod().when(taskService).validateTaskDetails(Mockito.any(RenovationTaskDTO.class), Mockito.any(RenovationRecord.class));
//    }
//
//    @Test
...
//        when(taskService.getTaskById(2L)).thenReturn(anotherTask);
//        mockMvc.perform(MockMvcRequestBuilders.get("/editTask")
//                        .contentType(MediaType.APPLICATION_FORM_URLENCODED)
//                        .param("taskId", "2")
